  ...
  config = mkIf (cfg.enable) (mkMerge [
    {
      zfs-root.fileSystems.datasets = {
        "zpool/nixos/home" = mkDefault "/home";
        "zpool/nixos/var/lib" = mkDefault "/var/lib";
        "zpool/nixos/var/log" = mkDefault "/var/log";
        "bpool/nixos/root" = "/boot";
      };
    }
    (mkIf (!cfg.immutable) {
      zfs-root.fileSystems.datasets = { "zpool/nixos/root" = "/"; };
    })
    (mkIf cfg.immutable {
      zfs-root.fileSystems = {
        datasets = {
          "zpool/nixos/empty" = "/";
          "zpool/nixos/root" = "/oldroot";
        };
        bindmounts = {
          "/oldroot/nix" = "/nix";
      ...
      boot.initrd.postDeviceCommands = ''
        if ! grep -q zfs_no_rollback /proc/cmdline; then
          ...
          zfs rollback -r zpool/nixos/empty@start
          zpool export -a
        fi
      '';
    })
      ...
      boot.initrd = {
        network = {
          ...
          postCommands = ''
            ...
            if zfs load-key zpool/nixos; then
               pkill zfs
            fi
            exit
            EOF'';
